package com.example.ajax.infractucture.rest.grps
...
class GrpcUserService(
    ...
    override fun delete(request: Mono<DeleteUserRequest>): Mono<DeleteUserResponse> {
        return request.flatMap {
            userServiceOutPort.deleteUser(it.userId).map { buildSuccessDeleteResponse() }
                .onErrorResume { exception ->
    ...
    override fun getAll(request: Mono<GetAllUsersRequest>): Mono<GetAllUsersResponse> {
        return userServiceOutPort.getAll(DEFAULT_LIMIT, DEFAULT_OFFSET).map { userMapper.mapToProto(it) }
    ...
    override fun stream(request: Mono<StreamRequest>): Flux<StreamResponse> =
        request.flatMapMany {
            userServiceOutPort.getById(it.userId)
                .flatMapMany { user ->
    ...
    private fun handleGetById(request: GetByIdUserRequest): Mono<GetByIdUserResponse> =
        userServiceOutPort.getById(request.userId)
